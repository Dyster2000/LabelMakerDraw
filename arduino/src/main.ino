...
#define WAITING_CONNECTION "Waiting for app "
#define WAITING_FOR_IMAGE  "Wait for image  "
#define RECEIVING_IMAGE    "Received    %   "
#define PRINTING_IMAGE     "Printing...     "

// printing variables
float x_scale = 7;      // Scale from 200x200 input to 960 steps for tape size
float y_scale = 7 * 2.8;
int space = 15;
...
const char *StateText[] = { WAITING_CONNECTION, WAITING_FOR_IMAGE, RECEIVING_IMAGE, PRINTING_IMAGE };
State currentState = WaitingForConnect;
...
LabelMakerStatus data;
LabelmakerBleServer bleServer(data);
...
void setup()
{
  ...
  bleServer.Init();
  Serial.println("Waiting for LabelMakerDraw app to connect...");
}
...
void loop()
{
  ...
  switch (currentState)
  {
    case WaitingForConnect:
      ...
      {
        Serial.println("State: WaitingForConnect -> WaitingForInput");
    ...
    case WaitingForInput:
      if (!bleServer.IsConnected())
      {
        Serial.println("State: WaitingForInput -> WaitingForConnect");
        currentState = WaitingForConnect;
      }
      else if (bleServer.IsReceivingImage())
      {
        Serial.println("State: WaitingForInput -> Receiving");
    ...
    case Receiving:
      if (!bleServer.IsConnected())
      {
        Serial.println("State: Receiving -> WaitingForConnect");
        currentState = WaitingForConnect;
      }
      else
      {
        auto percent = bleServer.ReceivedPercent();
        if (percent == 100)
        {
          Serial.println("State: Receiving -> Printing");
    ...
    {
      // plot image
      size_t imagePointCount = bleServer.GetImagePointCount();
      int highestX = 0;
      xpos = 0;
      ypos = 0;
      DrawPoint lastPoint;
      ...
      {
        auto nextPoint = bleServer.GetPoint(i);
        lastPoint = nextPoint;
